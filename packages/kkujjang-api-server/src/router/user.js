...
userRouter.get(
  ...
  async (req, res) => {
    ...
    const sessionId = await authSession.createSession({
      userId,
      kakaoToken: tokenData.access_token,
      authorityLevel,
    })

    console.log(JSON.stringify(await authSession.getSession(sessionId)))
...
userRouter.get('/auth-code', validateReceiverNumber, async (req, res) => {
  ...
  const smsAuthId = await smsAuthSession.createSession(
    authNumber,
...
userRouter.get('/signout', requireSignin, async (req, res) => {
  ...
  // 세션과 쿠키 삭제
  await authSession.destorySession(sessionId)
  res.setHeader(
    'Set-Cookie',
